...
struct mergeq {
	pthread_mutex_t mq_lock; /* Protects items below */
	pthread_cond_t	mq_cond; /* Condition variable */
	...
	pthread_t *mq_thrs;	/* Actual threads */
...
mergeq_fini(mergeq_t *mqp)
{
	...
	if (mqp->mq_ndthreads > 0) {
		mergeq_free(mqp->mq_thrs, sizeof (pthread_t) *
		    mqp->mq_ndthreads);
	}
	VERIFY0(pthread_cond_destroy(&mqp->mq_cond));
	VERIFY0(pthread_mutex_destroy(&mqp->mq_lock));
...
mergeq_init(mergeq_t **outp, uint_t nthrs)
{
	...
	if (mqp->mq_ndthreads > 0) {
		mqp->mq_thrs = mergeq_alloc(sizeof (pthread_t) *
		    mqp->mq_ndthreads);
		if (mqp->mq_thrs == NULL) {
	...
	if ((ret = pthread_mutex_init(&mqp->mq_lock, NULL)) != 0) {
		if (mqp->mq_ndthreads > 0) {
			mergeq_free(mqp->mq_thrs,
			    sizeof (pthread_t) * mqp->mq_ndthreads);
		}
		mergeq_free(mqp->mq_items, sizeof (void *) *
		    MERGEQ_DEFAULT_CAP);
		mergeq_free(mqp, sizeof (mergeq_t));
	...
	if ((ret = pthread_cond_init(&mqp->mq_cond, NULL)) != 0) {
		VERIFY0(pthread_mutex_destroy(&mqp->mq_lock));
		if (mqp->mq_ndthreads > 0) {
			mergeq_free(mqp->mq_thrs,
			    sizeof (pthread_t) * mqp->mq_ndthreads);
...
{
	VERIFY0(pthread_mutex_lock(&mqp->mq_lock));
	if (mqp->mq_working == B_TRUE) {
		VERIFY0(pthread_mutex_unlock(&mqp->mq_lock));
...
mergeq_push(mergeq_t *mqp, size_t slot, void *item)
{
	...
	while (mqp->mq_ncommit != slot && mqp->mq_iserror == B_FALSE)
		(void) pthread_cond_wait(&mqp->mq_cond, &mqp->mq_lock);
	...
	(void) pthread_cond_broadcast(&mqp->mq_cond);
}
    ...
    int *errp)
{
	...
	for (i = 0; i < mqp->mq_ndthreads; i++) {
		ret = pthread_create(&mqp->mq_thrs[i], NULL, mergeq_thr_merge,
		    mqp);
		if (ret != 0) {
			mqp->mq_iserror = B_TRUE;
			break;
	...
	for (i = 0; i < mqp->mq_ndthreads; i++) {
		VERIFY0(pthread_join(mqp->mq_thrs[i], NULL));
	}
