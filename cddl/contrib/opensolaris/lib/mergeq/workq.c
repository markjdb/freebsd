...
struct workq {
	pthread_mutex_t wq_lock; /* Protects below items */
	pthread_cond_t wq_cond;	/* Condition variable */
	...
	pthread_t *wq_thrs;	/* Actual threads */
...
workq_fini(workq_t *wqp)
{
	...
	VERIFY0(pthread_mutex_destroy(&wqp->wq_lock));
	VERIFY0(pthread_cond_destroy(&wqp->wq_cond));
	if (wqp->wq_cap > 0)
		workq_free(wqp->wq_items, sizeof (void *) * wqp->wq_cap);
	if (wqp->wq_ndthreads > 0)
		workq_free(wqp->wq_thrs, sizeof (pthread_t) *
		    wqp->wq_ndthreads);
...
workq_init(workq_t **outp, uint_t nthrs)
{
	...
	if (wqp->wq_ndthreads > 0) {
		wqp->wq_thrs = workq_alloc(sizeof (pthread_t) *
		    wqp->wq_ndthreads);
		if (wqp->wq_thrs == NULL) {
	...
	if ((ret = pthread_mutex_init(&wqp->wq_lock, NULL)) != 0) {
		if (wqp->wq_ndthreads > 0) {
			workq_free(wqp->wq_thrs,
			    sizeof (pthread_t) * wqp->wq_ndthreads);
		}
	...
	if ((ret = pthread_cond_init(&wqp->wq_cond, NULL)) != 0) {
		VERIFY0(pthread_mutex_destroy(&wqp->wq_lock));
		if (wqp->wq_ndthreads > 0) {
			workq_free(wqp->wq_thrs,
			    sizeof (pthread_t) * wqp->wq_ndthreads);
...
{
	VERIFY0(pthread_mutex_lock(&wqp->wq_lock));
	if (wqp->wq_working == B_TRUE) {
		VERIFY0(pthread_mutex_unlock(&wqp->wq_lock));
...
workq_work(workq_t *wqp, workq_proc_f *func, void *arg, int *errp)
{
	...
	for (i = 0; i < wqp->wq_ndthreads; i++) {
		ret = pthread_create(&wqp->wq_thrs[i], NULL, workq_thr_work,
		    wqp);
		if (ret != 0) {
			wqp->wq_iserror = B_TRUE;
		}
	...
	for (i = 0; i < wqp->wq_ndthreads; i++) {
		VERIFY0(pthread_join(wqp->wq_thrs[i], NULL));
	}
