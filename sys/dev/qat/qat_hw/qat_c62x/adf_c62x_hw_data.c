...
enum { DEV_C62X_SKU_1 = 0, DEV_C62X_SKU_2 = 1 };
...
get_accel_mask(struct adf_accel_dev *accel_dev)
{
	...
	straps = pci_read_config(pdev, ADF_C62X_SOFTSTRAP_CSR_OFFSET, 4);
...
get_ae_mask(struct adf_accel_dev *accel_dev)
{
	...
	fuse = pci_read_config(pdev, ADF_DEVICE_FUSECTL_OFFSET, 4);
	me_straps = pci_read_config(pdev, ADF_C62X_SOFTSTRAP_CSR_OFFSET, 4);

	/* If SSMs are disabled, then disable the corresponding MEs */
	ssms_disabled =
	    (~get_accel_mask(accel_dev)) & ADF_C62X_ACCELERATORS_MASK;
	me_disable = 0x3;
	while (ssms_disabled) {
		if (ssms_disabled & 1)
...
void
adf_init_hw_data_c62x(struct adf_hw_device_data *hw_data)
{
...
void
adf_clean_hw_data_c62x(struct adf_hw_device_data *hw_data)
{
