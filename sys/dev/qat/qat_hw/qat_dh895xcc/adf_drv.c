...
adf_probe(device_t dev)
{
	...
	for (id = adf_pci_tbl; id->vendor != 0; id++) {
		if (pci_get_vendor(dev) == id->vendor &&
			...
			device_set_desc(dev,
					"Intel " ADF_DH895XCC_DEVICE_NAME
					" QuickAssist");
...
static devclass_t adf_devclass;
DRIVER_MODULE_ORDERED(qat_dh895xcc, pci, adf_driver, adf_devclass, NULL, NULL,
		      SI_ORDER_THIRD);
MODULE_VERSION(qat_dh895xcc, 1);
MODULE_DEPEND(qat_dh895xcc, qat_common, 1, 1, 1);
MODULE_DEPEND(qat_dh895xcc, qat_api, 1, 1, 1);
MODULE_DEPEND(qat_dh895xcc, linuxkpi, 1, 1, 1);
