...
#define QAT_OCF_SESS_WATCHDOG_LOOPS (1000)
#define QAT_OCF_SESS_DELAY_IN_US (50)
...
/* Function protected by:
 * - when polling driven - by cyInstance mutex
 * - when IRQs driven - QAT IRQs are disabled during callback execution
 */
static
void
	      ...
	      CpaBoolean verifyResult)
{
	...
	if (CPA_STATUS_SUCCESS != status) {
		qat_cookie->opResult = CPA_STATUS_FAIL;
		rc = EIO;
		goto exit;
	}
	...
	/* Setting result */
	qat_cookie->opResult = status;
	qat_cookie->verifyResult = verifyResult;

	/* Verify result */
	if (CPA_STATUS_SUCCESS != qat_cookie->opResult) {
		rc = EBADMSG;
		goto exit;
	}

	/* Verify digest - no need to print log here */
	if (CPA_TRUE != qat_cookie->verifyResult) {
		rc = EBADMSG;
		goto exit;
	}
		     ...
		     struct qat_ocf_session *qat_ssession)
{
	...
	/* Cipher key */
	sessionSetupData.cipherSetupData.pCipherKey = csp->csp_cipher_key;
	...
	if (csp->csp_mode != CSP_MODE_CIPHER) {
		...
		if (NULL == pHashDefsInfo) {
			device_printf(
			    dev, "unable to find corresponding hash data\n");
			status = CPA_STATUS_UNSUPPORTED;
			goto fail;
		}
	...
	/* NOTE: lets keep double session approach to overcome lack of session
	 * update in FBSD QAT (unable to update e.g. direction)
	 */
		   ...
		   const struct crypto_session_params *csp)
{
	...
	if (NULL == qat_softc) {
		device_printf(dev, "Unable to extract qat_softc\n");
		return (EINVAL);
	}

	qat_instance =
	    &qat_softc->cyInstHandles[cpu_id % qat_softc->numCyInstances];
	if (NULL == qat_instance) {
		device_printf(dev, "no instance available\n");
		return (EINVAL);
	}

	if (NULL == qat_instance->cyInstHandle) {
		device_printf(dev, "no crypto instance available\n");
		return (EINVAL);
	}
	...
	explicit_bzero(&qat_dsession->encSession,
		       sizeof(qat_dsession->encSession));
	explicit_bzero(&qat_dsession->decSession,
		       sizeof(qat_dsession->decSession));
	mtx_unlock(&qat_instance->cyInstMtx);
		       ...
		       struct qat_ocf_session *qat_session)
{
	...
	int sessRemovalWatchdog = QAT_OCF_SESS_WATCHDOG_LOOPS;
	...
	do {
		 ...
		 */
		status = cpaCySymSessionInUse(qat_session->sessionCtx,
					      &sessionInUse);
		if (CPA_STATUS_SUCCESS != status) {
			device_printf(dev, "unable to get session status\n");
			return CPA_STATUS_FAIL;
		}
		if (CPA_FALSE == sessionInUse)
			break;
		if (sessRemovalWatchdog-- <= 0) {
			device_printf(dev,
				      "waiting for session un-busy timeout\n");
			return CPA_STATUS_FAIL;
		}
		DELAY(QAT_OCF_SESS_DELAY_IN_US);
	} while (TRUE);
	...
	explicit_bzero(qat_session->sessionCtx,
...
qat_ocf_freesession(device_t dev, crypto_session_t cses)
{
	...
	qat_dsession = crypto_get_driver_session(cses);
	if (NULL == qat_dsession) {
		device_printf(dev, "unable to find session to freed\n");
		return;
	}

	qat_instance = qat_dsession->qatInstance;
	if (NULL == qat_instance || NULL == qat_instance->cyInstHandle) {
		device_printf(dev, "no instance allocated for session\n");
		return;
	}
...
static
int
qat_ocf_handle_session_update(device_t dev,
			      struct cryptop *crp,
			      struct qat_ocf_instance *qat_instance,
			      struct qat_ocf_session *qat_session)
{
	CpaStatus status = CPA_STATUS_SUCCESS;
	CpaBoolean sessionInUse;
	int rc = 0;

	if (qat_session->aadLen == crp->crp_aad_length) {
		return 0;
	}

	mtx_lock(&qat_instance->cyInstMtx);
	status = cpaCySymSessionInUse(qat_session->sessionCtx, &sessionInUse);
	if (CPA_STATUS_SUCCESS != status) {
		device_printf(dev, "unable to get session in use status\n");
		rc = EIO;
		goto exit;
	}

	if (CPA_TRUE == sessionInUse) {
		device_printf(dev, "session busy - retry\n");
		rc = ERESTART;
		goto exit;
	}
	...
	status = qat_ocf_remove_session(dev,
					...
					qat_session);
	if (CPA_STATUS_SUCCESS != status) {
		device_printf(dev, "session removal failed\n");
		rc = EIO;
		goto exit;
	}

	status = qat_ocf_session_init(dev, crp, qat_instance, qat_session);
	if (CPA_STATUS_SUCCESS != status) {
		device_printf(dev, "session initialization failed\n");
		rc = EIO;
		goto exit;
...
qat_ocf_check_input(device_t dev, struct cryptop *crp)
{
	...
	if (CPA_TRUE == is_sep_aad_supported(csp) &&
		...
		device_printf(dev,
			      "AAD length out of range: %d max %d\n",
			      crp->crp_aad_length,
...
qat_ocf_process(device_t dev, struct cryptop *crp, int hint)
{
	...
	qat_dsession = crypto_get_driver_session(crp->crp_session);
	if (NULL == qat_dsession) {
		device_printf(dev, "unable to find session\n");
		rc = EIO;
		goto fail;
	}
	...
	if (NULL == qat_session->sessionCtx) {
		mtx_lock(&qat_instance->cyInstMtx);
		/* Double check in critical zone */
		if (NULL == qat_session->sessionCtx)
			status = qat_ocf_session_init(dev,
						      crp,
						      qat_instance,
						      qat_session);
		mtx_unlock(&qat_instance->cyInstMtx);
		...
		/* Function already returns errno style rcs */
		rc = qat_ocf_handle_session_update(dev,
						   crp,
						   qat_instance,
						   qat_session);
		if (rc)
	...
	/* Reset status flags */
	qat_cookie->opResult = CPA_STATUS_FAIL;
	qat_cookie->verifyResult = CPA_STATUS_FAIL;
...
qat_ocf_stop_instances(struct qat_ocf_softc *qat_softc)
{
	...
	if (NULL == qat_softc ||
	    NULL == qat_softc->cyInstHandles[0].cyInstHandle)
		return (EINVAL);
...
qat_ocf_attach(device_t dev)
{
	...
	qat_softc = device_get_softc(dev);
	if (!qat_softc) {
		device_printf(dev, "missing softc - cannot initialize!");
		goto fail;
	}
	...
	memset(qat_softc->cyInstHandles, 0, sizeof(qat_softc->cyInstHandles));
...
qat_ocf_detach(device_t dev)
{
	...
	if (qat_softc && qat_softc->cryptodev_id >= 0) {
	...
	/* Stop QAT instances */
	if (qat_softc) {
		status = qat_ocf_stop_instances(qat_softc);
		if (CPA_STATUS_SUCCESS != status)
...
static devclass_t qat_ocf_devclass;
