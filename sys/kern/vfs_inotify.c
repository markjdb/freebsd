...
inotify_close(struct file *fp, struct thread *td)
{
	...
	while ((rec = inotify_dequeue(sc)) != NULL) {
		if (rec != &sc->overflow)
			free(rec, M_INOTIFY);
	}
	mtx_unlock(&sc->lock);
...
inotify_queue_record(struct inotify_softc *sc, struct inotify_record *rec)
{
	...
	if (sc->npending >= inotify_max_queued_events || rec == &sc->overflow) {
    ...
    struct thread *td)
{
	...
	if (error != 0) {
		chginotifywatchcnt(sc->cred->cr_ruidinfo, -1, 0);
