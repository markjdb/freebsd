...
typedef struct ctf_die {
	...
	pthread_mutex_t	*cu_dwlock;	/* libdwarf lock */
...
#define	DWARF_LOCK(cup) \
	if ((cup)->cu_dwlock != NULL) \
		pthread_mutex_lock((cup)->cu_dwlock)
#define	DWARF_UNLOCK(cup) \
	if ((cup)->cu_dwlock != NULL) \
		pthread_mutex_unlock((cup)->cu_dwlock)
...
ctf_dwarf_preinit_dies(ctf_convert_t *cch, int fd, Elf *elf, Dwarf_Debug dw,
    pthread_mutex_t *dwlock, Dwarf_Error *derr, uint_t ndies, ctf_cu_t *cdies,
    char *errbuf, size_t errlen)
{
    ...
    char *errbuf, size_t errlen)
{
	...
	workq_t *wqp = NULL;
	pthread_mutex_t dwlock;

	*fpp = NULL;

	err = pthread_mutex_init(&dwlock, NULL);
	if (err != 0) {
		(void) snprintf(errbuf, errlen,
		    "mutex initialization failed: %d\n", err);
		return (err);
	}
