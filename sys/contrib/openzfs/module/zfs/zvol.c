...
zvol_set_common_sync_cb(dsl_pool_t *dp, dsl_dataset_t *ds, void *arg)
{
	...
	zsda->zsda_taskqid = taskq_dispatch(dp->dp_spa->spa_zvol_taskq,
	    zvol_task_cb, task, TQ_SLEEP);
    ...
    uint64_t val)
{
	...
	if (error == 0) {
		...
		mutex_enter(&zsda.zsda_lock);
		while (zsda.zsda_taskqid == TASKQID_INVALID)
			cv_wait(&zsda.zsda_cv, &zsda.zsda_lock);
		mutex_exit(&zsda.zsda_lock);

		error = spa_open(ddname, &spa, FTAG);
		if (error == 0) {
			taskq_wait_id(spa->spa_zvol_taskq, zsda.zsda_taskqid);
			spa_close(spa, FTAG);
		}
